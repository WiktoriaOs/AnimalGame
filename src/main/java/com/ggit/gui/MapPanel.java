...
import com.ggit.simulation.WorldMap;
...
public class MapPanel extends JPanel {
	...
	private void drawPlants(Graphics graphics) {
		...
		map.plantsLocations().keySet().forEach(position -> {
			leaf.translate(LayoutParams.MAP_SCALE * position.x(),
					LayoutParams.MAP_SCALE * position.y());
	...
	private void drawAnimals(Graphics graphics) {
		map.animalsMap().forEach((position, animals) -> {
			...
			int centerX = LayoutParams.MAP_SCALE * position.x() + LayoutParams.MAP_SCALE / 2;
			int centerY = LayoutParams.MAP_SCALE * position.y() + LayoutParams.MAP_SCALE / 2;
			graphics.fillOval(LayoutParams.MAP_SCALE * position.x(), // draw animal
					LayoutParams.MAP_SCALE * position.y(),
